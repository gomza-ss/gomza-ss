...
)
{
    ...
    const int detTokenMaxIdx = 5;
    if (tokens.size() < detTokenMaxIdx + 1)
    {
        errorMessage = "Too few tokens for MMR det: " + std::to_string(tokens.size());
        return false;
    }
...
REGISTER_COMMAND_BACK(detectmmr)
